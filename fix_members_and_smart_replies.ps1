...
# 3. Improve smart reply task to be more robust
...
def generate_smart_replies(room_id, user_id):
    ...
    try:
        ...
        # Format history with sender names
        history = []
...
Write-Host "✅ Updated smart reply task with better logging."

# 4. Improve services.py to generate suggestions for any conversation
$servicesPath = "apps/ai_assistant/services.py"
$servicesContent = Get-Content $servicesPath -Raw
# Ensure we use Gemini and have a good prompt
